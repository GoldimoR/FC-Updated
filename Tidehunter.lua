...
Tide.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Tidehunter"}, "1.Enable", "Turn on/off this script.")
Tide.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Tidehunter" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Tidehunter","Skills" }, "Kill Steal", "Auto cast Gush")
Tide.AddUltimate = Menu.AddOption({"Hero by Rasta", "Tidehunter","Skills"}, "Ultimate", "Use ultimate in combo")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Tidehunter","Skills"},"Gush","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Tidehunter","Skills"}, "Anchor Smash","Enable Skill")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Tidehunter", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Tidehunter", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Tidehunter", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Tidehunter"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Tide.npcName = "npc_dota_hero_tidehunter"


function Tide.OnUpdate()

	if not Menu.IsEnabled(Tide.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Tide.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Tide.KillSteal(myHero)
	end
	...
	if Menu.IsKeyDown(Tide.PressComboKey) then	
		-- Skills
		local gush = NPC.GetAbility(myHero, "tidehunter_gush")
		local smash = NPC.GetAbility(myHero, "tidehunter_anchor_smash")  
 		local ravage = NPC.GetAbility(myHero, "tidehunter_ravage")
		-- Items
        ...
        if  Menu.IsEnabled(Addskill1) and enemyTarget and gush and Ability.IsReady(gush) and Ability.IsCastable(gush, myMana) then
			Ability.CastTarget(gush, enemyTarget) 
		return end
		
        if  Menu.IsEnabled(Addskill2) and  smash and Ability.IsCastable(smash, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),360,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastNoTarget(smash) 
		return end
		...
		if ravage and Menu.IsEnabled(Tide.AddUltimate) and Ability.IsCastable(ravage, myMana) and Ability.IsReady(ravage) then
           Ability.CastNoTarget(ravage)
        end
...
function Tide.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_tidehunter" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "tidehunter_gush")
    if edge and Ability.IsCastable(edge, NPC.GetMana(myHero)) then
        edge_damage = Ability.GetLevelSpecialValueFor(edge, "gush_damage")
    end
...
function Tide.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "tidehunter_gush")
    if not edge or not Ability.IsCastable(edge, NPC.GetMana(myHero)) then return end
    local damage = Ability.GetLevelSpecialValueFor(edge, "gush_damage")
    local range = 670
