...
CM.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Crystal Maiden"}, "1.Enable", "Turn on/off this script.")
CM.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Crystal Maiden" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Crystal Maiden","Skills"}, "Kill Steal", "Auto cast Crystal Nova")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Crystal Maiden","Skills"},"Crystal Nova","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Crystal Maiden","Skills"}, "Frostbite","Enable Skill")
local Addskill3 = Menu.AddOption({"Hero by Rasta", "Crystal Maiden","Skills"}, "Ultimate", "Use ultimate on combo")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Crystal Maiden", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Crystal Maiden", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local AddGlimmer = Menu.AddOptionBool({ "Hero by Rasta", "Crystal Maiden", "Items" }, "Glimmer", false)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Crystal Maiden", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Crystal Maiden"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
CM.npcName = "npc_dota_hero_crystal_maiden"


function CM.OnUpdate()

	if not Menu.IsEnabled(CM.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= CM.npcName then return end
	...
	if Menu.IsKeyDown(CM.PressComboKey) then	
		-- Skills
		local q = NPC.GetAbility(myHero, "crystal_maiden_crystal_nova")
		local w = NPC.GetAbility(myHero, "crystal_maiden_frostbite")  
 		local r = NPC.GetAbility(myHero, "crystal_maiden_freezing_field")	  
		-- Items
...
return			
		...
		if r and Menu.IsEnabled(Addskill3) and Ability.IsCastable(r, myMana) and Ability.IsReady(r) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),400,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		...
		if not Ability.IsActivated(r) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1300,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
...
function CM.OnDraw()
    ...
    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "crystal_maiden_crystal_nova")
...
function CM.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "crystal_maiden_crystal_nova")
    if not edge or not Ability.IsCastable(edge, NPC.GetMana(myHero)) then return end
    local damage = Ability.GetLevelSpecialValueFor(edge, "nova_damage") - 25
    local range = 650
