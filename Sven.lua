...
Sven.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Sven"}, "1.Enable", "Turn on/off this script.")
Sven.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Sven" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
Sven.AddUltimate = Menu.AddOption({"Hero by Rasta", "Sven","Skills"}, "Ultimate", "Use ultimate on combo")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Sven","Skills"}, "Kill Steal", "Auto cast Storm Bolt")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Sven","Skills"},"Storm Bolt","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Sven","Skills"}, "Warcry","Enable Skill")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Sven", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Sven", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Sven", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Sven"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Sven.npcName = "npc_dota_hero_sven"


function Sven.OnUpdate()

  hp = Menu.GetValue(Sven.AddSatanic)

	if not Menu.IsEnabled(Sven.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Sven.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Sven.KillSteal(myHero)
	end
	...
	if Menu.IsKeyDown(Sven.PressComboKey) then	
		-- Skills
		local bolt = NPC.GetAbility(myHero, "sven_storm_bolt")
		local warcry = NPC.GetAbility(myHero, "sven_warcry")  
 		local gods = NPC.GetAbility(myHero, "sven_gods_strength")
		-- Items
		...
		if Menu.IsEnabled(Addskill2) and  enemyTarget and warcry and Ability.IsReady(warcry) and Ability.IsCastable(warcry, myMana) then
			Ability.CastNoTarget(warcry) 
		end
		
		if gods and Menu.IsEnabled(Sven.AddUltimate) and Ability.IsCastable(gods, myMana) and Ability.IsReady(gods) then
        Ability.CastNoTarget(gods)
        return 
		end		
		...
		if Menu.IsEnabled(Addskill1) and  enemyTarget and bolt and Ability.IsReady(bolt) and Ability.IsCastable(bolt, myMana) then
...
function Sven.OnDraw()
    ...
    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_sven" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "sven_storm_bolt")
...
function Sven.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "sven_storm_bolt")
