...
local optionEnableScript = Menu.AddOption({"Hero by Rasta", "Beastmaster"}, "1.Enable", "Turn on/off this script.")
local PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Beastmaster" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local Addabillity1 = Menu.AddOption({"Hero by Rasta", "Beastmaster","Skills"}, "Wild Axes","Enable Skill")
local Addabillity2 = Menu.AddOption({"Hero by Rasta", "Beastmaster","Skills"}, "Call of the Wild","Enable Skill") 
local Addabillity3 = Menu.AddOption({"Hero by Rasta", "Beastmaster","Skills"}, "Ultimate Primal Roar","Enable Skill")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Beastmaster", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Beastmaster", "Items"}, "Auto Satanic", 400, 2000, 1000)
--local Addurna = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Soul Ring","Use item on combo")
--local Addspirit = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Pipe of Insigth","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Scythe of Vyse","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Heavens Halberd","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "BKB","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Manta Style","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Blade Mail","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Force Staff","Use item on combo")
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Hurricane pike","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Blink","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Beastmaster", "Items" }, "Eul","Use item on combo")
...
function beast.OnUpdate()
	...
	if not Menu.IsEnabled(optionEnableScript) then return true end
		...
		if Menu.IsEnabled(PressComboKey) then
		beast.attackHero()
		return
	end	

	if Menu.GetValue(Addethereal) == 1 then
	beast.Ethereal(myHero)
	end
    ...
    return
		...
		local callofwild = NPC.GetAbility(myHero, "beastmaster_call_of_the_wild_boar")  
...
--		local spiritvessel = NPC.GetItem(myHero,"item_spirit_vessel", true)
		...
		if Menu.IsEnabled (Addabillity1) and axes and Ability.IsCastable(axes, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastPosition(axes,enemyAbsOrigin) 
		end
		
		if Menu.IsEnabled (Addabillity2) and enemyTarget and callofwild and Ability.IsReady(callofwild) and Ability.IsCastable(callofwild, myMana) then 
			Ability.CastNoTarget(callofwild) 
		end
		
		if Menu.IsEnabled (Addabillity3) and enemyTarget and RoR and Ability.IsReady(RoR) and Ability.IsCastable(RoR, myMana) then 
...
function beast.Ethereal(myHero)
