...
WKing.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Wraith King"}, "1.Enable", "Turn on/off this script.")
WKing.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Wraith King" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Wraith King","Skills"}, "Kill Steal", "Hellfire Blast")
WKing.AddUltimate = Menu.AddOption({"Hero by Rasta", "Wraith King","Skills"}, "Skeletons", "Use Mortal Strike on combo")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Wraith King","Skills"},"Hellfire Blast","Enable Skill")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Wraith King", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Wraith King", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Wraith King", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Wraith King"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)


WKing.npcName = "npc_dota_hero_skeleton_king"
function WKing.OnUpdate()
	if not Menu.IsEnabled(WKing.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= WKing.npcName then return end
	
	if Menu.IsEnabled(optionKillSteal) then
    WKing.KillSteal(myHero)
	end
	...
	if Menu.IsKeyDown(WKing.PressComboKey) then	
		-- Skills
		local wraithfireblast = NPC.GetAbility(myHero, "skeleton_king_hellfire_blast")  
		local mortalstrike = NPC.GetAbility(myHero, "skeleton_king_mortal_strike")
		-- Items		
		...
		if Menu.IsEnabled(Addskill1) and enemyTarget and wraithfireblast and Ability.IsReady(wraithfireblast) and Ability.IsCastable(wraithfireblast, myMana) then
			Ability.CastTarget(wraithfireblast, enemyTarget) 
		end
		
		if mortalstrike and Menu.IsEnabled(WKing.AddUltimate) and Ability.IsCastable(mortalstrike, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),360,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastNoTarget(mortalstrike) 
		end
...
function WKing.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_skeleton_king" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "skeleton_king_hellfire_blast")
    if edge and Ability.IsCastable(edge, NPC.GetMana(myHero)) then
        edge_damage = Ability.GetDamage(edge)
    end
...
function WKing.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "skeleton_king_hellfire_blast")
