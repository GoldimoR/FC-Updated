...
Brist.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Bristleback"}, "1.Enable", "Turn on/off this script.")
Brist.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Bristleback" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Bristleback","Skills"},"Viscous Nasal Goo","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Bristleback","Skills"}, "Quill Spray","Enable Skill")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Bristleback","Skills"}, "Kill Steal", "Auto cast Quill Spray")
local auto2skill = Menu.AddOption({"Hero by Rasta", "Bristleback","Skills"}, "Auto 2 Skill", "Auto cast Quill Spray if enemy in radius 600")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Bristleback", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Bristleback", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Bristleback", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Bristleback"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Brist.npcName = "npc_dota_hero_bristleback"

function Brist.OnUpdate()

    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Brist.npcName then return end
	
	local enemyTarget = Input.GetNearestHeroToCursor(Entity.GetTeamNum(myHero), Enum.TeamType.TEAM_ENEMY)

	if not Menu.IsEnabled(Brist.optionEnableScript) then return true end

    if Menu.IsEnabled(optionKillSteal) then
    Brist.KillSteal(myHero)
	end
	...
	if Menu.IsEnabled(auto2skill) then
	local quill = NPC.GetAbilityByIndex(myHero, 1)
	local myMana = NPC.GetMana(myHero)
		
	if quill and Ability.IsCastable(quill, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),600,0) then 
	...
	if Menu.IsKeyDown(Brist.PressComboKey) then	
		-- Skills
		local q = NPC.GetAbility(myHero, "bristleback_viscous_nasal_goo")
		local w = NPC.GetAbility(myHero, "bristleback_quill_spray")  
		...
		if Menu.IsEnabled (Addskill2) and w and Ability.IsCastable(w, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),600,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
...
function Brist.OnDraw()
    ...
    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "bristleback_quill_spray")
...
function Brist.KillSteal(myHero)
    local edge = NPC.GetAbility(myHero, "bristleback_quill_spray")
    if not edge or not Ability.IsCastable(edge, NPC.GetMana(myHero)) then return end
    local damage = Ability.GetLevelSpecialValueFor(edge,"quill_base_damage") + Ability.GetLevelSpecialValueFor(edge,"quill_stack_damage")
    local range = 330
    local enemies = NPC.GetHeroesInRadius(myHero, range, Enum.TeamType.TEAM_ENEMY)
    if not enemies or #enemies <= 0 then return end

    for i, enemy in ipairs(enemies) do
	    local enemyHPregen = NPC.GetHealthRegen (enemy) * 2
