...
Bane.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Bane"}, "1.Enable", "Turn on/off this script.")
Bane.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Bane" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Bane","Skills"}, "Kill Steal", "Auto cast Brain Sap")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Bane","Skills"},"Enfeeble","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Bane","Skills"}, "Brain Sap","Enable Skill")
local Addskill3 = Menu.AddOption({"Hero by Rasta", "Bane","Skills"}, "Nightmare", "Use Nightmare on combo")
local Addskill4 = Menu.AddOption({"Hero by Rasta", "Bane","Skills"}, "Ultimate", "Use ultimate on combo")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Bane", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Bane", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Bane", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Bane"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Bane.npcName = "npc_dota_hero_bane"


function Bane.OnUpdate()

	if not Menu.IsEnabled(Bane.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Bane.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Bane.KillSteal(myHero)
	end
	...
	if Menu.IsKeyDown(Bane.PressComboKey) then	
		-- Skills
		local q = NPC.GetAbility(myHero, "bane_enfeeble")
		local w = NPC.GetAbility(myHero, "bane_brain_sap")  
 		local e = NPC.GetAbility(myHero, "bane_fiends_grip")
 		local r = NPC.GetAbility(myHero, "bane_nightmare")	  
		-- Items
		...
		if Menu.IsEnabled(Addskill3) and  enemyTarget and e and Ability.IsReady(e) and Ability.IsCastable(e, myMana) then
		...
		if Menu.IsEnabled(Addskill4) and  enemyTarget and r and Ability.IsReady(r) and Ability.IsCastable(r, myMana) then
...
function Bane.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_Bane" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "bane_brain_sap")
...
function Bane.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "bane_brain_sap")
    if not edge or not Ability.IsCastable(edge, NPC.GetMana(myHero)) then return end
    local damage = Ability.GetLevelSpecialValueFor(edge, "brain_sap_damage") - 20
    local range = 550
