...
local optionEnableScript = Menu.AddOption({"Hero by Rasta", "Bounty Hunter"}, "1.Enable", "Turn on/off this script.")
local PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Bounty Hunter" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Bounty Hunter"}, "Awareness", "Show how many hits left to kill enemy")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Bounty Hunter","Skills"},"Cast Shuriken","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Bounty Hunter","Skills"}, "Cast Track","Enable Skill")
local optionAutoTrack = Menu.AddOption({"Hero by Rasta", "Bounty Hunter","Skills"}, "Auto Track", "Auto cast track on enemies in range or use track to break linken")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Bounty Hunter","Skills"}, "Kill Steal", "Auto cast Shuriken")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Bounty Hunter", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Bounty Hunter", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Hurricane Pike", "if target in radius 400")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "BKB", "if target in radius 800")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Blade Mail", "if target in radius 800")
local Addblink = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Scythe of Vyse","Use item on combo")
--local Addurna = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Soul Ring","Use item on combo")
--local Addspirit = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Bounty Hunter", "Items" }, "Eul","Use item on combo")
...
function Hunter.OnUpdate()
	...
	if Menu.IsKeyDown(PressComboKey) then	
		...
		if Menu.IsEnabled(Addskill2) and enemyTarget and track and Ability.IsReady(track) and Ability.IsCastable(track, myMana) then
...
--		end
		...
		if Menu.IsEnabled(Addskill2) and enemyTarget and shuriken and Ability.IsReady(shuriken) and Ability.IsCastable(shuriken, myMana) then
...
function Hunter.Ethereal(myHero)
...
function Hunter.AutoTrack()
    ...
    for i = 1, Heroes.Count() do
        ...
        and Hunter.CanCastSpellOn(enemy) and NPC.IsEntityInRange(myHero, enemy, range)
        and (Hunter.GetTrackTimeLeft(enemy) <= Ability.GetCooldownLength(spell)
...
function Hunter.GetTrackTimeLeft(npc)
...
function Hunter.IsSuitableToCastSpell(myHero)
...
function Hunter.CanCastSpellOn(npc)
