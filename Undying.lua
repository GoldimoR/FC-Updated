...
Undyng.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Undying"}, "1.Enable", "Turn on/off this script.")
Undyng.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Undying" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Undying","Skills"}, "Kill Steal", "Auto cast Decay")
Undyng.AddTombstone = Menu.AddOption({"Hero by Rasta", "Undying","Skills"}, "Tombstone", "Use Tombstone on combo")
Undyng.AddUltimate = Menu.AddOption({"Hero by Rasta", "Undying","Skills"}, "Ultimate", "Use ultimate on combo")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Undying","Skills"},"Battery Assault","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Undying","Skills"}, "Power Cogs","Enable Skill")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Undying", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Undying", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Undying", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Undying"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Undyng.npcName = "npc_dota_hero_undying"


function Undyng.OnUpdate()

	if not Menu.IsEnabled(Undyng.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Undyng.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Undyng.KillSteal(myHero)
	end
	...
	if Menu.IsKeyDown(Undyng.PressComboKey) then	
		-- Skills
		local decay = NPC.GetAbility(myHero, "undying_decay")
		local rip = NPC.GetAbility(myHero, "undying_soul_rip")  
 		local tombstone = NPC.GetAbility(myHero, "undying_tombstone")
 		local golem = NPC.GetAbility(myHero, "undying_flesh_golem")		
		-- Items
		...
		if Menu.IsEnabled(Addskill1) and  decay and Ability.IsCastable(decay, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastPosition(decay,enemyAbsOrigin)
		end 
		
		if Menu.IsEnabled(Addskill2) and  enemyTarget and rip and Ability.IsReady(rip) and Ability.IsCastable(rip, myMana) then
			Ability.CastTarget(rip, enemyTarget) 
		end
		
		if golem and Menu.IsEnabled(Undyng.AddUltimate) and Ability.IsCastable(golem, myMana) and Ability.IsReady(golem) then
           Ability.CastNoTarget(golem)
        end
		
		if tombstone and Menu.IsEnabled(Undyng.AddTombstone) and Ability.IsCastable(tombstone, myMana) and Ability.IsReady(tombstone) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),550,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastPosition(tombstone,enemyAbsOrigin) 
        end
...
function Undyng.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_undying" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "undying_decay")
...
function Undyng.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "undying_decay")
