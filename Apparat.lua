...
Apparat.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Ancient Apparition"}, "1.Enable", "Turn on/off this script.")
Apparat.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Ancient Apparition" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Ancient Apparition","Skills"}, "Kill Steal", "Auto cast Chilling Touch")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Ancient Apparition","Skills"},"Cold Feet","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Ancient Apparition","Skills"}, "Ice Vortex","Enable Skill")
local Addskill3 = Menu.AddOption({"Hero by Rasta", "Ancient Apparition","Skills"}, "Chilling Touch", "Use Tombstone on combo")
local Addskill4 = Menu.AddOption({"Hero by Rasta", "Ancient Apparition","Skills"}, "Ice Blast", "Use ultimate on combo")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Ancient Apparition", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Ancient Apparition", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Ancient Apparition", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Ancient Apparition"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Apparat.npcName = "npc_dota_hero_ancient_apparition"


function Apparat.OnUpdate()

	if not Menu.IsEnabled(Apparat.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Apparat.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Apparat.KillSteal(myHero)
	end
	...
	if Menu.IsKeyDown(Apparat.PressComboKey) then	
		-- Skills
		local q = NPC.GetAbility(myHero, "ancient_apparition_cold_feet")
		local w = NPC.GetAbility(myHero, "ancient_apparition_ice_vortex")  
 		local e = NPC.GetAbility(myHero, "ancient_apparition_chilling_touch")
 		local r = NPC.GetAbility(myHero, "ancient_apparition_ice_blast")
		local r2 = NPC.GetAbility(myHero, "ancient_apparition_ice_blast_release")
		-- Items
		...
		if Menu.IsEnabled(Addskill2) and  enemyTarget and w and Ability.IsReady(w) and Ability.IsCastable(w, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastPosition(w,enemyAbsOrigin)
		end
		
		if Menu.IsEnabled(Addskill1) and  q and Ability.IsCastable(q, myMana) and  Ability.IsReady(q) then
		Ability.CastTarget(q ,enemyTarget)
		end 
		
		
		if Menu.IsEnabled(Addskill3) and  e and Ability.IsCastable(e, myMana) and  Ability.IsReady(e) then
		Ability.CastTarget(e ,enemyTarget)
		end 
		
		if Menu.IsEnabled(Addskill4) and  enemyTarget and r and Ability.IsReady(r) and Ability.IsCastable(r, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
...
function Apparat.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_ancient_apparition" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "ancient_apparition_chilling_touch")
    if edge and Ability.IsCastable(edge, NPC.GetMana(myHero)) then
        edge_damage = Ability.GetLevelSpecialValueFor(edge, "damage") + NPC.GetTrueDamage (myHero)
    end
...
function Apparat.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "ancient_apparition_chilling_touch")
    if not edge or not Ability.IsCastable(edge, NPC.GetMana(myHero)) then return end
    local damage = Ability.GetLevelSpecialValueFor(edge, "damage") + NPC.GetTrueDamage (myHero)
    local range = 630
