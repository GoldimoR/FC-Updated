...
Trent.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Treant Protector"}, "1.Enable", "Turn on/off this script.")
Trent.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Treant Protector" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
Trent.AddUltimate = Menu.AddOption({"Hero by Rasta", "Treant Protector","Skills"}, "Ultimate", "Use ultimate on combo")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Treant Protector","Skills"},"Leech Seed","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Treant Protector","Skills"}, "Living Armor","Enable Skill")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Treant Protector", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Treant Protector", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Treant Protector", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Treant Protector"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Trent.npcName = "npc_dota_hero_treant"


function Trent.OnUpdate()

	if not Menu.IsEnabled(Trent.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Trent.npcName then return end
	...
	if Menu.IsKeyDown(Trent.PressComboKey) then	
		-- Skills
		local seed = NPC.GetAbility(myHero, "treant_leech_seed")
		local armor = NPC.GetAbility(myHero, "treant_living_armor")  
 		local overgrowth = NPC.GetAbility(myHero, "treant_overgrowth")
		-- Items
		...
		if enemyTarget and armor and Ability.IsReady(armor) and Ability.IsCastable(armor, myMana) then
			Ability.CastTarget(armor, myHero) 
		return end
		
		if enemyTarget and seed and Ability.IsReady(seed) and Ability.IsCastable(seed, myMana) then
			Ability.CastTarget(seed, enemyTarget) 
		return end
       ...
       if Menu.IsEnabled (Addspirit) and spiritvessel and enemyTarget and Ability.IsReady(spiritvessel) and Ability.IsCastable(spiritvessel, myMana) then
		...
		if overgrowth and Menu.IsEnabled(Trent.AddUltimate) and Ability.IsCastable(overgrowth, myMana) and Ability.IsReady(overgrowth)and
        enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),700,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
           Ability.CastNoTarget(overgrowth)
        end
...
function Trent.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_treant" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "treant_leech_seed")
