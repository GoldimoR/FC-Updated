...
Vendeful.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit"}, "1.Enable", "Turn on/off this script.")
Vendeful.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Vengeful Spirit" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit","Skills"}, "Kill Steal", "Auto cast Magic Missile")
Vendeful.AddUltimate = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit","Skills"}, "Ultimate", "Use ultimate on combo")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit","Skills"},"Magic Missile","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit","Skills"}, "Wave of Terror","Enable Skill")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Vengeful Spirit", "Items", "Auto Satanic"}, "hp for acrivation", 400, 2000, 1000)
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Hurricane Pike if target in radius 400","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "BKB if target in radius 800","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Blade Mail if target in radius 800","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Blink","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Scythe of Vyse","Use item on combo")
local Addurna = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Soul Ring","Use item on combo")
local Addspirit = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Pipe of Insigth","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Heavens Halberd","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Manta Style","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Force Staff","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit", "Items" }, "Eul","Use item on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Vengeful Spirit"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Vendeful.npcName = "npc_dota_hero_vengefulspirit"


function Vendeful.OnUpdate()

	if not Menu.IsEnabled(Vendeful.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Vendeful.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Vendeful.KillSteal(myHero)
	end
	...
	if Menu.IsKeyDown(Vendeful.PressComboKey) then	
		-- Skills
		local missile = NPC.GetAbility(myHero, "vengefulspirit_magic_missile")
		local terror = NPC.GetAbility(myHero, "vengefulspirit_wave_of_terror")  
 		local swap = NPC.GetAbility(myHero, "vengefulspirit_nether_swap")
		-- Items
		...
		if swap and Menu.IsEnabled(Vendeful.AddUltimate) and Ability.IsCastable(swap, myMana) and Ability.IsReady(swap) then
			Ability.CastTarget(swap, enemyTarget) 
        end
		
		if Menu.IsEnabled (Addskill1) and enemyTarget and missile and Ability.IsReady(missile) and Ability.IsCastable(missile, myMana) then
			Ability.CastTarget(missile, enemyTarget) 
		end
		
		if Menu.IsEnabled (Addskill2) and  terror and Ability.IsCastable(terror, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastPosition(terror,enemyAbsOrigin) 
		end
...
function Vendeful.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_vengefulspirit" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "vengefulspirit_magic_missile")
    if edge and Ability.IsCastable(edge, NPC.GetMana(myHero)) then
        edge_damage = Ability.GetLevelSpecialValueFor(edge, "magic_missile_damage")
    end
...
function Vendeful.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "vengefulspirit_magic_missile")
    if not edge or not Ability.IsCastable(edge, NPC.GetMana(myHero)) then return end
    local damage = Ability.GetLevelSpecialValueFor(edge, "magic_missile_damage")
    local range = 490
