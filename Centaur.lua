...
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Centaur","Skills"}, "Kill Steal", "Auto cast Edge")
local Addskill1 = Menu.AddOption({"Hero by Rasta", "Centaur","Skills"},"Hoof Stomp","Enable Skill")
local Addskill2 = Menu.AddOption({"Hero by Rasta", "Centaur","Skills"}, "Double Edge","Enable Skill")
local Addskill3 = Menu.AddOption({"Hero by Rasta", "Centaur","Skills"},"Retaliate","Enable Skill")
local Addskill4 = Menu.AddOption({"Hero by Rasta", "Centaur","Skills"}, "Stampede","Enable Skill")
local Addethereal = Menu.AddOptionCombo({"Hero by Rasta", "Centaur", "Items" }, "Ethereal Blade", {"Use in Combo", "Just Kill Steal"}, 1)
local AddSatanic = Menu.AddOptionSlider({"Hero by Rasta", "Centaur", "Items"}, "Auto Satanic", 400, 2000, 1000)
--local Addurna = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Urn of Shadows","Use item on combo")
local Addsoulring = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Soul Ring","Use item on combo")
--local Addspirit = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Spirit Vessel","Use item on combo")
local Addpipe = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Pipe of Insigth","Use item on combo")
local Addskythe = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Scythe of Vyse","Use item on combo")
local Addcrimpson = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Crimson Guard","Use item on combo")
local Addhalebard = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Heavens Halberd","Use item on combo")
local AddBKB = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "BKB","Use item on combo")
local Addshivas = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Shivas Guard","Use item on combo")
local Addmjollnir = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Mjollnir","Use item on combo")
local Addatos = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Rod of Atos","Use item on combo")
local AddMoM = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Mask of Madnes","Use item on combo")
local Addnecronomicon = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Necronomicon","Use item on combo")
local Addveilofdiscord = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Veil of Discord","Use item on combo")
local Addlotusorb = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Lotus Orb","Use item on combo")
local Addmanta = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Manta Style","Use item on combo")
local Addblademail = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Blade Mail","Use item on combo")
local Adddiffusalblade = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Diffusal Blade","Use item on combo")
local Addnullifier = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Nullifier","Use item on combo")
local Addmedallion = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Medallion of Courage","Use item on combo")
local Addsolarcrest = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Solar Crest","Use item on combo")
local Addorchid = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Orchid","Use item on combo")
local Addbloodthorn = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Bloodthorn","Use item on combo")
local Addabyssalblade = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Abyssal Blade","Use item on combo")
local Addforcestaff = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Force Staff","Use item on combo")
local Addhurricanepike = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Hurricane pike","Use item on combo")
local Addblink = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Blink","Use item on combo")
local Addeul = Menu.AddOption({"Hero by Rasta", "Centaur", "Items" }, "Eul","Use item on combo")
...
function Centaur.OnUpdate()
	...
	if Menu.IsKeyDown(Centaur.PressComboKey) then	
		...
		local turn = NPC.GetAbility(myHero, "centaur_return")
		local stampede = NPC.GetAbility(myHero, "centaur_stampede")
		--Items
...
--		local spiritvessel = NPC.GetItem(myHero,"item_spirit_vessel", true)
		...
		if Menu.IsEnabled(Addskill1) and enemyTarget and stomp and Ability.IsReady(stomp) and Ability.IsCastable(stomp, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),300,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
			Ability.CastNoTarget(stomp)
		end
		
		if Menu.IsEnabled(Addskill2) and enemyTarget and edge and Ability.IsReady(edge) and Ability.IsCastable(edge, myMana) then 
			Ability.CastTarget(edge, enemyTarget) 
		end
		
		if Menu.IsEnabled(Addskill3) and enemyTarget and turn and Ability.IsReady(turn) and Ability.IsCastable(turn, myMana) then
			Ability.CastNoTarget(turn)
		end
		
		if Menu.IsEnabled(Addskill4) and enemyTarget and stampede and Ability.IsReady(stampede) and Ability.IsCastable(stampede, myMana) then
			Ability.CastNoTarget(stampede)
		end
...
function Centaur.Ethereal(myHero)
    ...
    local damage = Hero.GetPrimaryAttribute(myHero) * 2 + 75
    local range = 800
    local enemies = NPC.GetHeroesInRadius(myHero, range, Enum.TeamType.TEAM_ENEMY)
    if not enemies or #enemies <= 0 then return     end
